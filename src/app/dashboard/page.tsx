...
import { Users, Sparkles, AlertTriangle, TrendingUp, ArrowRight, ExternalLink, CheckCircle, XCircle } from "lucide-react"
...
export default function DashboardPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          ...
          <h1 className="display-text text-4xl font-light text-ivory">
            {hotel ? hotel.name : hotelLoading ? "Loading…" : "Not Registered"}
          </h1>
          <p className="text-ivory-dim text-sm mt-1 mono-text">{address ? formatAddress(address) : "—"}</p>
        </div>
        <div className="flex items-center gap-3">
          {hotel ? (
            <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded border border-success/20 bg-success/5 text-success text-xs mono-text">
              <CheckCircle className="w-3.5 h-3.5" /> Registered
            </span>
          ) : !hotelLoading ? (
            <Link href="/dashboard/settings" className="btn-gold text-sm">
              Register Hotel
            </Link>
          ) : null}
        </div>
        ...
        {statCards.map((s) => {
          ...
          return (
            <Link
              key={s.label}
              href={s.href}
              className="glass-card rounded-xl p-5 hover:border-gold/20 transition-all group"
            >
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="space-y-3">
              {[
                ...
                ["Loyalty Tiers", hotel.loyalty_tiers?.join(", ") || "—"],
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {hotel.room_types?.map((r) => (
                    <span key={r} className="mono-text text-gold-dim border border-border-gold rounded px-2 py-0.5">{r}</span>
                  ))}
                </div>
              </div>
              <div>
        ...
        <div className="space-y-2">
          {[
            ...
            <Link
              key={item.step}
              href={item.href}
              className="flex items-center gap-4 px-4 py-3 rounded hover:bg-card-hi transition-colors group"
            >
              <span className="w-6 h-6 rounded-full border border-gold/20 bg-gold/5 flex items-center justify-center shrink-0 mono-text text-gold">
                {item.step}
              </span>
      ...
      {/* Contract badge */}
      <div className="glass-card rounded-xl p-6 border border-gold/10">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          <div className="flex-1">
            ...
            <p className="text-ivory-dim text-xs">
              All recommendations are validated by independent GenLayer validators via multi-consensus
              on StudioNet. Every verdict is immutable and on-chain.
            </p>
          </div>
          <a
            ...
            className="btn-outline shrink-0 inline-flex items-center gap-2 text-xs"
          >
