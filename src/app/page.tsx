    ...
    <div className="min-h-screen bg-espresso text-ivory">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 border-b border-border/50 bg-espresso/80 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="display-text text-2xl font-light tracking-wide text-ivory">Comfot</div>
          <div className="flex items-center gap-6">
            <a href={studioContractLink(GENLAYER_CONTRACT_ADDRESS)} target="_blank" rel="noopener noreferrer"
            ...
            <Link href="/connect" className="btn-gold text-sm px-5 py-2">Launch Console</Link>
          </div>
        </div>
      </nav>
      ...
      <section className="relative pt-40 pb-28 px-6 overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
          <Image
            src={unsplash(SITE_IMAGES.hero, 1920, 1080)}
            alt="Luxury hotel"
            fill
        ...
        <div className="relative z-10 max-w-5xl mx-auto text-center">
          <motion.div
            ...
            className="inline-flex items-center gap-2 border border-gold/20 bg-gold/5 text-gold text-xs font-medium px-4 py-2 rounded-full mb-10 mono-text"
          >
          ...
          <motion.h1
            ...
            className="display-text text-6xl md:text-8xl font-light text-ivory leading-none mb-6"
          >
          ...
          >
            <Link href="/connect" className="btn-gold inline-flex items-center gap-2 text-base px-8 py-3.5">
      ...
      <section className="py-24 px-6 border-t border-border">
        <div className="max-w-6xl mx-auto">
          ...
          <StaggerSection className="grid md:grid-cols-2 gap-6 mb-10">
            {[
              {
                label: "Guest A says:",
              ...
              <StaggerItem key={g.label}>
                <motion.div whileHover={{ y: -4, transition: { duration: 0.2 } }} className="glass-card rounded-2xl p-8 h-full">
      ...
      <section className="py-24 px-6 bg-panel border-t border-border">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="space-y-4">
            {[
              { step: "01", icon: <Hotel className="w-5 h-5 text-gold" />, title: "Register your hotel", desc: "Connect your wallet on StudioNet. Register your hotel — room types, amenities, category — directly on-chain. No database." },
              { step: "02", icon: <Users className="w-5 h-5 text-gold" />, title: "Submit guest profiles", desc: "Upload loyalty tier, review excerpts, special requests, dietary needs, and conversation history. Profiles are stored on the intelligent contract." },
              { step: "03", icon: <Sparkles className="w-5 h-5 text-gold" />, title: "Request a recommendation", desc: "Select a guest. The contract runs LLM inference over all guest signals to generate a room, amenity, and package recommendation with full justification." },
              { step: "04", icon: <Shield className="w-5 h-5 text-gold" />, title: "Validator consensus decides", desc: "Multiple independent GenLayer validators each run their own LLM inference and vote. The eq_principle mechanism compares outputs and produces a consensus verdict: APPROVED, REJECTED, or ESCALATED." },
              { step: "05", icon: <CheckCircle className="w-5 h-5 text-gold" />, title: "Act on the verdict", desc: "Approved? Send the recommendation to your team. Escalated? Review and decide in the dashboard. All scores, reasoning, and flags are stored immutably on-chain." },
            ].map((item, i) => (
              <FadeUp key={item.step} delay={i * 0.07}>
                <div className="flex gap-6 items-start">
                  <div className="flex-shrink-0 flex flex-col items-center">
                    <motion.div whileHover={{ scale: 1.1 }} className="w-10 h-10 rounded-xl bg-gold/10 border border-gold/20 flex items-center justify-center">
                      {item.icon}
                    </motion.div>
                    {i < 4 && <div className="w-px h-full min-h-[2rem] bg-border mt-2" />}
                  </div>
                  <motion.div whileHover={{ x: 4, transition: { duration: 0.2 } }} className="glass-card rounded-xl px-6 py-5 flex-1">
      ...
      <section className="py-24 px-6 border-t border-border">
        <div className="max-w-6xl mx-auto">
          <FadeUp className="text-center mb-16">
            ...
            <h2 className="display-text text-4xl md:text-5xl font-light text-ivory">Why existing solutions fall short</h2>
          </FadeUp>
          <StaggerSection className="grid md:grid-cols-3 gap-6">
            {[
              { title: "Rule-based systems", flaw: "Cannot handle nuance. Spa user + early riser = different slots, different packages, different floors. Rules cannot capture this." },
              { title: "Keyword matching", flaw: "Quiet is not a feature. It is a feeling. No keyword maps cleanly to the right room, amenity, or pre-arrival message." },
              { title: "Single AI model", flaw: "One model, one interpretation. No independent verification. No consensus. No way to know whether the output is actually aligned with the guest." },
            ].map((item) => (
              <StaggerItem key={item.title}>
                <motion.div whileHover={{ y: -4, transition: { duration: 0.2 } }} className="glass-card rounded-xl p-6 border border-danger/10 h-full">
      ...
      <section className="py-24 px-6 bg-panel border-t border-border">
        <div className="max-w-6xl mx-auto">
          <FadeUp className="text-center mb-16">
            ...
            <h2 className="display-text text-4xl md:text-5xl font-light text-ivory">GenLayer validates semantic alignment</h2>
          </FadeUp>
          <StaggerSection className="grid md:grid-cols-3 gap-6">
            {[
              { icon: <Zap className="w-5 h-5 text-gold" />, title: "Non-deterministic inference", desc: "The contract runs LLM inference on reviews, loyalty data, and conversation history to extract granular preference tags and generate a full recommendation with justification." },
              { icon: <Shield className="w-5 h-5 text-gold" />, title: "Multi-validator consensus", desc: "Independent GenLayer validators each judge semantic alignment. The eq_principle mechanism compares their outputs. Consensus decides — not a single model." },
              { icon: <Eye className="w-5 h-5 text-gold" />, title: "Immutable on-chain verdict", desc: "Every decision — with dimension scores, validator flags, alignment score, and full reasoning — is stored permanently on StudioNet. Auditable. Transparent." },
            ].map((item) => (
              <StaggerItem key={item.title}>
                <motion.div whileHover={{ y: -4, transition: { duration: 0.2 } }} className="glass-card rounded-xl p-6 border border-gold/10 h-full">
                  <div className="w-10 h-10 rounded-lg bg-gold/10 flex items-center justify-center mb-4">{item.icon}</div>
                  <h3 className="text-ivory font-medium mb-3">{item.title}</h3>
                  <p className="text-ivory-dim text-sm leading-relaxed">{item.desc}</p>
                </motion.div>
              </StaggerItem>
      ...
      <section className="py-24 px-6 border-t border-border">
        <div className="max-w-6xl mx-auto">
          <FadeUp className="text-center mb-16">
            ...
            <h2 className="display-text text-4xl font-light text-ivory">Three possible verdicts</h2>
            <p className="text-ivory-dim text-sm mt-3 max-w-lg mx-auto">Every recommendation goes through consensus. There are only three outcomes — no silent defaults, no hallucinated certainty.</p>
          </FadeUp>
          <StaggerSection className="grid md:grid-cols-3 gap-6">
            <StaggerItem>
      ...
      <section className="py-24 px-6 bg-panel border-t border-border">
        <div className="max-w-4xl mx-auto">
          <FadeUp className="text-center mb-12">
            ...
            <p className="text-ivory-dim text-sm max-w-lg mx-auto">No database. No API server. No backend logic. The GenLayer Intelligent Contract is the single source of truth.</p>
          </FadeUp>
          <FadeUp>
            <div className="glass-card rounded-2xl p-8 font-mono text-sm space-y-1.5">
              {[
                ...
                { text: `ComfotContract.py · StudioNet · ${GENLAYER_CONTRACT_ADDRESS.slice(0,14)}…`, color: "text-gold" },
      ...
      <section className="py-24 px-6 border-t border-border">
        <div className="max-w-6xl mx-auto">
          <FadeUp className="text-center mb-16">
            ...
            <h2 className="display-text text-4xl font-light text-ivory">Everything a hotel operator needs</h2>
          </FadeUp>
      ...
      <section className="relative py-28 px-6 border-t border-border overflow-hidden">
        <div className="absolute inset-0 z-0">
          <Image src={unsplash(SITE_IMAGES.ctaBg, 1920, 600)} alt="" fill className="object-cover object-center" />
        ...
        <div className="relative z-10 max-w-2xl mx-auto text-center">
          <FadeUp>
            ...
            <h2 className="display-text text-5xl md:text-6xl font-light text-ivory mb-6">
              Validate comfort.<br /><span className="italic text-gold">Not guess it.</span>
            </h2>
            ...
            <Link href="/connect" className="btn-gold inline-flex items-center gap-2 text-base px-10 py-4">
              Open Hotel Console <ArrowRight className="w-4 h-4" />
            </Link>
      ...
      <footer className="border-t border-border px-6 py-8">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4 text-ivory-dim text-xs">
          ...
          <div className="flex items-center gap-6">
            <a href="https://docs.genlayer.com" target="_blank" rel="noopener noreferrer" className="hover:text-gold transition-colors">GenLayer Docs</a>
            <a href={studioContractLink(GENLAYER_CONTRACT_ADDRESS)} target="_blank" rel="noopener noreferrer" className="mono-text hover:text-gold transition-colors">
